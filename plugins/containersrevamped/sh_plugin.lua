...
-- properties
properties.Add("container_enable", {
    ...
    Filter = function(self, entity, client)
        ...
        if (!gamemode.Call("CanProperty", client, "container_enable", entity)) then return false end
    ...
    Receive = function(self, length, client)
        ...
        if (data) then
			...
			ix.log.AddType("containerMake", function(client, target)
				return string.format("%s has made a container!", client:GetName())
			end)
			ix.log.Add(client, "containerMake", client)
            entity:Remove()
        end
    end
})
